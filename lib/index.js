...
module.exports = {
	...
	// write Model Service APi template
	...
	// write wx html ejs template
	writeWxHtml(wxdir, name) {
		var T = Template.wxHtml(name)
		var dirName = wxdir || 'wx/views'
		try {
			fs.writeFileSync(`${dirName}/${name}.ejs`, T.EJS, 'utf8')
		} catch (err) {
			console.log(err)
		}
	}
